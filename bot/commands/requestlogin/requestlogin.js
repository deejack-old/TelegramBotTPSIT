...
class BanCommand extends Command {
    ...
    async onCommand(message) {
        ...
        botService.bot.sendMessage(message.from.id,
            `La tua password temporanea è: ${password}, il tuo id utente è ${user.id}, l'id del gruppo è ${group.id} <a href=\"t.me/c/${message.chat.id.toString().substr(message.chat.id.toString().length - 10, 10)}/${message.message_id}\">richiesta</a>`, {
                parse_mode: 'HTML'
            })
    }
}
